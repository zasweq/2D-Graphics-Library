...
public:
    ...
    void rebuildShader() {
        switch (fShaderType) {
            ...
            case kBitmap_ShaderType: {
                GMatrix mx;
                mx.setScale((fGradPts[1].fX - fGradPts[0].fX) / gBM.width(),
                            (fGradPts[1].fY - fGradPts[0].fY) / gBM.height());
                mx.postTranslate(fGradPts[0].fX, fGradPts[0].fY);
...
protected:
    ...
    void drawShaderHilite(GCanvas* canvas) {
        GColor hilite_color = {1, 0, 0, 0};    // black for gradient
        switch (fShaderType) {
            case kColor_ShaderType:
                break;
            case kBitmap_ShaderType:
                hilite_color = {1, 1, 0, 0};
                // fall-through
            case kGradient_ShaderType:
                draw_line(canvas, fGradPts[0], fGradPts[1], hilite_color, 1.4f);
