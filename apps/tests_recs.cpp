...
static bool bitmap_pix_eq(const GBitmap& bitmap, GPixel inside_width, GPixel outside_width) {
    const int lastX = bitmap.rowBytes() >> 2;
    const GPixel* row = bitmap.pixels();

    for (int y = 0; y < bitmap.height(); ++y) {
        // Check the pixels within the logical width
        for (int x = 0; x < bitmap.width(); ++x) {
            if (row[x] != inside_width) {
                return false;
            }
        }
        // Check the "pixels" that are to the right of our "right" edge, but within rowbytes
        for (int x = bitmap.width(); x < lastX; ++x) {
            if (row[x] != outside_width) {
                return false;
            }
        }
        row += lastX;
...
static void test_clear(GTestStats* stats) {
    int w = 10, h = 10;
    size_t rb = (w + 11) * sizeof(GPixel);  // deliberately make rowbytes bigger than needed
    size_t size = rb * h;
    GBitmap bitmap(w, h, rb, (GPixel*)malloc(size), GBitmap::kNo_IsOpaque);

    const int wacky_component = 123;

    // This will set all pixels to 123, even those outside of our logical width
    ...
    EXPECT_TRUE(stats, mem_eq(bitmap.pixels(), wacky_component, size));

    const GPixel wacky_pixel = GPixel_PackARGB(wacky_component, wacky_component,
                                               wacky_component, wacky_component);

    // This should draw zeros, but only inside the logical width
    canvas->clear(GColor::MakeARGB(0, 1, 1, 1));
    // Ensure the zeros are inside the logical width, but not outside (to the right)
    EXPECT_TRUE(stats, bitmap_pix_eq(bitmap, 0, wacky_pixel));

    // Same test, but with opaque-white
    canvas->clear(GColor::MakeARGB(1, 1, 1, 1));
    EXPECT_TRUE(stats, bitmap_pix_eq(bitmap, 0xFFFFFFFF, wacky_pixel));
}
...
static void test_rect_colors(GTestStats* stats) {
    ...
    EXPECT_TRUE(stats, check9(bitmap, nine));
...
static void test_bad_input_poly(GTestStats* stats) {
    ...
    EXPECT_TRUE(stats, is_filled_with(surface.bitmap(), white));
